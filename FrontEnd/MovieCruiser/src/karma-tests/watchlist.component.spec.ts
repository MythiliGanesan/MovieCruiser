...
describe('WatchlistComponent - retrieval of watchlist', () => {
	...
	beforeEach(() => {
		...
		mockBackend.connections.subscribe(connection => {
			if (
				connection.request.url.startsWith(
	...
	it(
		'same number of results should be rendered on the page as retrieved from the api',
		fakeAsync(() => {
			fixture.detectChanges();
			tick();
			fixture.detectChanges();
			let movieList = fixture.debugElement.queryAll(By.css('.moviePoster'));
			expect(movieList.length).toBe(
				3,
				`If there are three movies in watchlist, there should be
      ...
      three input elements on watchlist page decorated with class-name: 'movieCommentsInpt'`
			...
			movieList = fixture.debugElement.queryAll(
	...
	it(
		'every list item should be showing up right in class:movieDescription',
		fakeAsync(() => {
			fixture.detectChanges();
			tick();
			fixture.detectChanges();
			const movieList = fixture.debugElement.queryAll(
				By.css('.movieDescription')
			);
			expect(movieList[0].nativeElement.innerText).toBe(
				'Le Carton (1945)',
				`The element under
      first movie item in watchlist decorated with class-name:movieDescription is not
      displaying correctly`
			);
			expect(movieList[1].nativeElement.innerText).toBe(
				'Blade Runner 2049 ()',
				`The element under
      second movie item in watchlist decorated with class-name:movieDescription is not
      displaying correctly`
			);
			expect(movieList[2].nativeElement.innerText).toBe(
				'It (2001)',
				`The element under
	...
	it(
		'every list item should be showing up right in class:moviePoster',
		fakeAsync(() => {
			fixture.detectChanges();
			tick();
			fixture.detectChanges();
			const movieList = fixture.debugElement.queryAll(By.css('.moviePoster'));
			expect(movieList[0].properties.src).toBe(
				'https://image.tmdb.org/t/p/w500/4uH7XT0xB6ilhZe0UhgfGzHAmgz.jpg',
				`The image element under first movie item in watchlist decorated with class-name:moviePoster is not pointing
      to the right source`
			);
			expect(movieList[1].properties.src).toBe(
      ...
      to the right source`
			...
			expect(movieList[2].properties.src).toBe(
	...
	it(
		'every list item should be showing up right in class:movieRating',
		fakeAsync(() => {
			fixture.detectChanges();
			tick();
			fixture.detectChanges();
	...
	it(
		'every list item should be showing up right in class:movieCommentsInpt',
		fakeAsync(() => {
			fixture.detectChanges();
			tick();
			fixture.detectChanges();
			...
			expect(movieList[0].nativeElement.value).toBe(
				'',
				`The comments input element under
      first movie item in watchlist decorated with class-name:movieCommentsInpt should be empty`
			);
			expect(movieList[1].nativeElement.value).toBe(
				'I would like to watch this complete',
				`The
      ...
      class-name:movieCommentsInpt should be set correctly`
			...
			expect(movieList[2].nativeElement.value).toBe(
				'',
				`The comments input element under
      first movie item in watchlist decorated with class-name:movieCommentsInpt should be empty`
			);
...
describe('WatchlistComponent - navigation to movie details page', () => {
			...
			TestBed.configureTestingModule({
				...
				imports: [
					...
					RouterTestingModule.withRoutes([
						{ path: 'movie/33704', component: Dummy },
	...
	it(
		'every list item image click should switch to the movie detail page for right movieId',
		fakeAsync(() => {
			fixture.detectChanges();
			tick();
			fixture.detectChanges();
			...
			expect(location.path()).toBe(
				'/movie/33704',
				`Clicking on the image element of
...
describe('WatchlistComponent - updation of comments', () => {
	...
	it(
		'first list item update comment button should pick the corresponding comment text and invoke the right api',
		fakeAsync(() => {
			mockBackend.connections.subscribe(connection => {
				if (
					connection.request.url.startsWith(
	...
	it(
		'second list item update comment button should pick the corresponding comment text and invoke the right api',
		fakeAsync(() => {
			mockBackend.connections.subscribe(connection => {
				if (
					connection.request.url.startsWith(
	...
	it(
		'third list item update comment button should pick the corresponding comment text and invoke the right api',
		fakeAsync(() => {
			mockBackend.connections.subscribe(connection => {
				if (
					connection.request.url.startsWith(
...
describe('WatchlistComponent - removal from watchlist', () => {
	...
	it(
		'first list item remove button should invoke right api to remove the item',
		fakeAsync(() => {
			mockBackend.connections.subscribe(connection => {
				if (
					connection.request.url.startsWith(
			...
			tick();
			fixture.detectChanges();
			const movieRemoveBtns = fixture.debugElement.queryAll(
				By.css('.movieRemoveBtn')
			);
			movieRemoveBtns[0].nativeElement.click();
		})
	);
	it(
		'second list item remove button should invoke right api to remove the item',
		fakeAsync(() => {
			mockBackend.connections.subscribe(connection => {
				if (
					connection.request.url.startsWith(
	...
	it(
		'third list item remove button should invoke right api to remove the item',
		fakeAsync(() => {
			mockBackend.connections.subscribe(connection => {
				if (
					connection.request.url.startsWith(
